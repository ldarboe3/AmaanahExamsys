...
- **Mobile Packet API (Amaanah Examiner App Integration)**: REST endpoints under `/api/mobile/packets` for the external Amaanah Examiner mobile app (Expo React Native). Staff identified by X-Staff-ID: {EID} header (no session auth required). Endpoints: `GET /api/mobile/packets` (list+filter), `GET /api/mobile/packets/stats` (role-scoped packet statistics with phase and detailed status counts), `GET /api/mobile/packets/:barcode` (with full scan history), `GET /api/mobile/packets/:barcode/action?role=` (next available action per role), `POST /api/mobile/packets/scan` (submit scan, advance status, reject duplicates). `/api/mobile/packets/stats` accepts query params: `examCenter`, `cluster`, `region` for scope override; returns `{total, dispatching, atCenter, returning, completed, byStatus}` where phases are dispatching (at_hq→dispatched_to_center), atCenter (at_center/opened/sealed), returning (returned_to_*), and byStatus contains count for each individual status. Role→action→status mapping: hq_staff dispatches/receives_return; regional_coordinator receives/dispatches/forwards_return; cluster_officer receives/dispatches/receives_return/forwards_return; examiner receives/opens/seals/dispatch_return. Scan history stored in `mobile_packet_scans` table.
