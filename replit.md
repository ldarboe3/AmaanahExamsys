...
- **Exam Paper Logistics & Tracking (Phase 2 – Event-Based)**: Rebuilt around a `packetEvents` table (event types: packed, dispatched, received, opened, sealed, return_dispatched, return_received, archived). Web dashboard (`/packet-tracking`) is now monitoring-only with stats overview (Total, Packed, In Transit, At Location, At Center, Active, Returned), filterable packet list, chain-of-custody timeline modal, and a barcode look-up tab. Mobile PWA (`/mobile-packet-scan`) is the field operations interface: scan-first barcode input, role-based action buttons, offline IndexedDB queue with auto-sync, GPS capture, sync status indicator, recent activity log, and a **live stats strip** at the top showing Total / Dispatching / At Locations / Active / Returned counts (refreshed every minute from `/api/packet-events/dashboard/stats`). Backward-compatible with existing `handover_logs` + `exam_packets` tables.
- **Exam Scheduling & Time Enforcement**: Allows HQ to create and publish exam schedules. Facilitates recording of actual exam start/end times by examiners, with automatic detection and logging of late starts and ends, including reason codes. A HQ monitoring dashboard provides real-time oversight. Mobile timetable displays exam schedules as clickable cards with subject names (resolved from ID → name mapping), grouped by date (today first, then future, then past), with real-time active exam indicator. Tapping a card opens a slide-up sheet showing subject details (name, grade, date, time window), Exam Packets section with barcode, center name, and received/not-received status badges (green if packet reached at_center/opened/sealed or return stage; amber if in transit). Empty state shown when no packets created.
...
- **Student Attendance Scanning (Phase 6)**: Subject-based attendance tracking split into two interfaces: (1) Mobile Scanner (`/mobile-attendance-scan`) — mobile-PWA optimized barcode scanning page for examiners and field staff; offline IndexedDB queue with auto-sync, vibration feedback, GPS capture, per-subject selection, recent scan log with sync status badges. Subject picker shows **today's exams only** (filtered by the backend to today's published schedules for the center's grades) with a "Today only · [date]" badge, exam time displayed alongside subject name, and a clear empty state when no exams are scheduled today. (2) Admin Monitoring Dashboard (`/student-attendance`) — admin-only two-tab page: Monitoring tab (attendance rates by region/cluster/center/subject), Validation Flags tab (three inconsistency types: attended-but-no-marks, marks-but-no-attendance, passing-marks-but-no-attendance). Examiners see the mobile scanner in their sidebar. (3) **Center Attendance Roster** — the Attendance tab on `/centers/{id}` now shows a live table for admin/HQ users of all students assigned to that center with columns: Student Name, Index No., School, Subject, Exam Status (green tick=present / red X=absent / gray dash=not scanned), and scan time. Fetches from `GET /api/attendance/center-records` via a CTE SQL join (students→schools→attendance_records→subjects). Auto-refreshes every 30 seconds. Searchable/filterable by name, school, grade, and status. Summary stats strip shows total, present, absent, and not-scanned counts.
...
- **Mobile Packet API (Amaanah Examiner App Integration)**: REST endpoints under `/api/mobile/` for the external Amaanah Examiner mobile app (Expo React Native). **Auth**: All mobile endpoints accept EITHER `X-Staff-ID: {EID}` header OR `Authorization: Bearer {EID}:{confirmationCode}` (staff card barcode token). The Bearer token uses the EID and the confirmationCode stored in the staff profile (same value embedded in the physical staff card barcode). Endpoints: `GET /api/mobile/packets` (list+filter), `GET /api/mobile/packets/stats` (role-scoped packet statistics), `GET /api/mobile/packets/:barcode` (with full scan history), `GET /api/mobile/packets/:barcode/action?role=` (next available action per role), `POST /api/mobile/packets/scan` (submit scan, advance status). Packet responses include multiple field aliases for center/cluster/region names: `examCenter`/`centerName`, `cluster`/`clusterName`, `region`/`regionName` (plus `centerId`, `clusterId`, `regionId`). **Lookup endpoints**: `GET /api/mobile/centers` (all centers with regionName/clusterName), `GET /api/mobile/centers/:id` (single center detail), `GET /api/mobile/regions`, `GET /api/mobile/clusters` — all support Bearer token auth. Role→action→status mapping: hq_staff dispatches/receives_return; regional_coordinator receives/dispatches/forwards_return; cluster_officer receives/dispatches/receives_return/forwards_return; examiner receives/opens/seals/dispatch_return. Scan history stored in `mobile_packet_scans` table.
  - **Public Mobile Timetable API**: Two unauthenticated endpoints for the mobile app timetable: (1) `GET /api/public/subjects` — returns all subjects as `[{id, name, grade}]`; (2) `GET /api/public/exam-schedules` — returns published schedules enriched with `subjectName`. Supports query params: `examYearId`, `grade`, `centerId`, `clusterId`, `regionId`, `includePackets=true`. When `includePackets=true`, each schedule includes a `packets` array with `{id, barcode, centerName, centerId, status, received}`.
  - **Mobile Attendance Count API**: `GET /api/mobile/attendance/count` (auth: `X-Staff-ID: {EID}` or Bearer token). Returns attendance totals scoped to the caller's role. Query params: `subject`, `examYearId`, plus scope: `center`, `cluster`, `region`. Response: `{total, scopeLabel, subjectName, centerBreakdown: [{center, centerId, count}]}`.
  - **Key Files**: server/routes.ts (mobile packet API section + resolveStaffFromRequest helper + public subjects/schedules), server/storage.ts (createMobilePacketScan, getMobilePacketScans), shared/schema.ts (mobilePacketScans)
