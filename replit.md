...
- **Student Attendance Scanning (Phase 6)**: Subject-based attendance tracking with three-tab interface: (1) Scan tab — barcode scanning per subject with offline IndexedDB queue, instant feedback, and automatic sync; (2) Monitoring tab — attendance summary by region/cluster/center/subject with attendance rates; (3) Validation Flags tab — automated cross-checking of attendance vs marks records, flagging three inconsistency types: attended-but-no-marks, marks-but-no-attendance, passing-marks-but-no-attendance. Admin roles see all three tabs; examiners see only the Scan tab.
  - **Key Files**: client/src/pages/student-attendance.tsx, server/routes.ts (attendance-scan section + /api/attendance/monitoring-summary + /api/attendance/validation-flags), shared/schema.ts (attendanceRecords), server/storage.ts
