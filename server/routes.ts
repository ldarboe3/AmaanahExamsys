...
${pages.map(p => `  <url>
      ...
      for (let i = 0; i < schoolsData.length; i++) {
        ...
        try {
          ...
          const [createdSchool] = await db.insert(schools).values({
            ...
            adminUserId: user.id, // Link admin user to school
          }).returning();
  ...
  app.get("/api/students/exam-cards/pdf", isAuthenticated, async (req, res) => {
    try {
      ...
      let filteredStudents = await storage.getAllStudents();
      ...
      if (user.role === 'school_admin') {
        ...
        filteredStudents = filteredStudents.filter(s => s.schoolId === userSchoolId);
      } else if (schoolId) {
      ...
      filteredStudents = filteredStudents.filter(s => s.status === 'approved' && s.indexNumber);
      
      if (examYearId) {
        filteredStudents = filteredStudents.filter(s => s.examYearId === examYearId);
      }
      ...
      if (cardDataList.length === 1) {
        pdfBuffer = await generateStudentExamCard(cardDataList[0]);
      } else {
        pdfBuffer = await generateBulkStudentExamCards(cardDataList);
      }
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          // If school not found - mark as unmatched (NO creation)
          if (!matchedSchool) {
            if (!processedSchoolNames.has(normalizedSchoolName)) {
              // Track unmatched school for reporting
            ...
            // Mark row as unmatched school - will be skipped
            previewRows.push({
              rowNumber: i + 2,
              rawSchoolName: schoolName.trim(),
          ...
          if (!studentId) {
            // Student not found - mark as unmatched (NO creation)
      ...
      console.log(`[Preview] Complete: ${rows.length} rows, ${matchedRows} matched, ${matchedSchoolIds.size} schools matched, ${existingStudentsCount} students matched, ${unmatchedSchoolsCount} schools not found, ${unmatchedStudentsCount} students not found`);
  ...
  app.post("/api/results/upload/confirm", isAuthenticated, async (req, res) => {
    try {
      ...
      // Only process rows where both school AND student were matched (NO creation)
      const processableRows = previewData.rows.filter(r => r.matchStatus === 'matched' && r.studentId && r.marks.length > 0);
      
      console.log(`[Confirm] Processing ${processableRows.length} matched rows for results upload (no new students/schools created)`);
      ...
      console.log(`[Confirm] Applied: ${resultsCreated} results created, ${resultsUpdated} results updated for ${processableRows.length} matched students`);
  ...
  app.get("/api/certificates/eligible-students", isAuthenticated, async (req, res) => {
    try {
      ...
      // Filter to only show students WITH results (for certificate generation)
      const studentsWithResults = eligibleStudents.filter(s => s.hasResults);
      
      // Apply pagination
      const totalCount = studentsWithResults.length;
      const paginatedStudents = studentsWithResults.slice(offset, offset + limit);
      ...
      res.json({
        ...
        summary: {
          ...
          eligible: studentsWithResults.filter(s => s.isEligible).length,
          withCertificate: studentsWithResults.filter(s => s.hasCertificate).length,
          missingData: studentsWithResults.filter(s => s.missingFields.length > 0).length,
          noResults: 0, // All students shown have results
          failed: studentsWithResults.filter(s => !s.passed).length,
        }
      });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
...
100001,ISLAMIC,30,55`;
  ...
  app.post("/api/center-assignments/auto-assign", isAuthenticated, async (req, res) => {
    try {
      ...
      // Only include schools with at least one approved student for this exam year
      const allStudents = await storage.getAllStudents();
      const schoolsWithApprovedStudents = new Set(
        allStudents
          .filter(s => s.examYearId === examYearId && s.status === 'approved')
      ...
      // Build school→center self-assignment map (school IS a center)
      const schoolCenterMap = new Map<number, typeof allCenters[0]>();
      for (const center of allCenters) {
        if ((center as any).schoolId && center.isActive) {
          schoolCenterMap.set((center as any).schoolId, center);
        }
      }

      // Track center student counts from already-assigned schools
      ...
      for (const school of unassignedSchools) {
        ...
        if (bestCenter) {
          ...
          const schoolStudents = await storage.getStudentsBySchool(school.id);
          const addedCount = schoolStudents.filter(s =>
            s.examYearId === examYearId && s.status === 'approved'
          ).length;
          centerStudentCounts[bestCenter.id] = (centerStudentCounts[bestCenter.id] || 0) + addedCount;
      ...
      // ── Auto-generate halls (40 students per hall) ────────────────────────
      const STUDENTS_PER_HALL = 40;

      for (const centerId of centersWithNewAssignments) {
        const totalStudents = centerStudentCounts[centerId] || 0;
        if (totalStudents === 0) continue;

        const hallsNeeded = Math.ceil(totalStudents / STUDENTS_PER_HALL);

        // Remove previously auto-generated halls (named "Hall N")
        const existingHalls = await storage.getCenterHallsByCenter(centerId);
        const autoHalls = existingHalls.filter(h => /^Hall \d+$/i.test(h.name));
        for (const hall of autoHalls) {
          await storage.deleteCenterHall(hall.id);
        }

        // Create halls
        for (let i = 1; i <= hallsNeeded; i++) {
          await storage.createCenterHall({ centerId, name: `Hall ${i}`, capacity: STUDENTS_PER_HALL });
        }
  ...
  // ─── CSV Bulk Upload for Examination Centers ─────────────────────────────
  const centerCsvUpload = multer({
    storage: multer.memoryStorage(),
    limits: { fileSize: 5 * 1024 * 1024 },
    fileFilter: (_req: any, file, cb) => {
      if (file.originalname.endsWith('.csv') || file.mimetype === 'text/csv') cb(null, true);
  ...
  const parseCenterCsvRows = (csvText: string, allRegions: any[], allClusters: any[], allCenters: any[]) => {
    ...
    // Resolve region + cluster from a row using multiple column strategies
    const resolveRegionCluster = (row: Record<string, string>, sortedRegions: any[]): { targetRegion: any; targetCluster: any; errorMsg?: string } => {
      ...
      if (regionIdRaw) {
        const rId = parseInt(regionIdRaw);
        const targetRegion = allRegions.find((r: any) => r.id === rId);
        if (!targetRegion) return { targetRegion: null, targetCluster: null, errorMsg: `Region ID ${regionIdRaw} not found` };
        let targetCluster: any = null;
        if (clusterIdRaw) {
          const cId = parseInt(clusterIdRaw);
          targetCluster = allClusters.find((c: any) => c.id === cId && c.regionId === targetRegion.id);
          if (!targetCluster) return { targetRegion, targetCluster: null, errorMsg: `Cluster ID ${clusterIdRaw} not found in region "${targetRegion.name}"` };
        } else {
          targetCluster = allClusters.filter((c: any) => c.regionId === targetRegion.id).sort((a: any, b: any) => a.id - b.id)[0];
          if (!targetCluster) return { targetRegion, targetCluster: null, errorMsg: `No cluster found for region "${targetRegion.name}"` };
        }
        return { targetRegion, targetCluster };
      }

      // Strategy 3: region (by name) + cluster (by name, optional)
      const regionNameRaw = row['region'] || row['region_name'] || '';
      const clusterNameRaw = row['cluster'] || row['cluster_name'] || '';
      if (regionNameRaw) {
        const targetRegion = allRegions.find((r: any) => norm(r.name) === norm(regionNameRaw))
          ?? allRegions.find((r: any) => norm(r.name).includes(norm(regionNameRaw)) || norm(regionNameRaw).includes(norm(r.name)));
        if (!targetRegion) return { targetRegion: null, targetCluster: null, errorMsg: `Region "${regionNameRaw}" not found` };
        const regionClusters = allClusters.filter((c: any) => c.regionId === targetRegion.id).sort((a: any, b: any) => a.id - b.id);
        let targetCluster: any = null;
        if (clusterNameRaw) {
          targetCluster = regionClusters.find((c: any) => norm(c.name) === norm(clusterNameRaw))
            ?? regionClusters.find((c: any) => norm(c.name).includes(norm(clusterNameRaw)) || norm(clusterNameRaw).includes(norm(c.name)));
          if (!targetCluster) return { targetRegion, targetCluster: null, errorMsg: `Cluster "${clusterNameRaw}" not found in region "${targetRegion.name}"` };
        } else {
          targetCluster = regionClusters[0];
          if (!targetCluster) return { targetRegion, targetCluster: null, errorMsg: `No cluster found for region "${targetRegion.name}"` };
        }
        return { targetRegion, targetCluster };
      }

      return { targetRegion: null, targetCluster: null, errorMsg: 'Missing region/cluster — add a region, region_id, or region_cluster column' };
    };
  ...
  app.post("/api/centers/bulk-upload-csv", isAuthenticated, (req, res) => {
    centerCsvUpload(req, res, async (err) => {
      ...
      try {
        ...
        const allRegions = await storage.getAllRegions();
        const allClusters = await storage.getAllClusters();
        const sortedRegions = [...allRegions].sort((a, b) => a.id - b.id);
        ...
        for (let i = 1; i < rawLines.length; i++) {
          ...
          // Resolve region + cluster using multiple column strategies
          const norm = (s: string) => (s || '').trim().toLowerCase();
          ...
          if (effectiveRc) {
            ...
            const clusterIdRaw = row['cluster_id'] || '';
            if (regionIdRaw) {
              targetRegion = allRegions.find(r => r.id === parseInt(regionIdRaw));
              if (!targetRegion) { regionError = `Region ID ${regionIdRaw} not found`; }
              else if (clusterIdRaw) {
                targetCluster = allClusters.find(c => c.id === parseInt(clusterIdRaw) && c.regionId === targetRegion.id);
                if (!targetCluster) regionError = `Cluster ID ${clusterIdRaw} not found in region "${targetRegion.name}"`;
              } else {
                targetCluster = allClusters.filter(c => c.regionId === targetRegion.id).sort((a, b) => a.id - b.id)[0];
              ...
              if (regionNameRaw2) {
                ...
                if (!targetRegion) { regionError = `Region "${regionNameRaw2}" not found`; }
                  ...
                  if (clusterNameRaw) {
                    targetCluster = rc.find(c => norm(c.name) === norm(clusterNameRaw))
                      ?? rc.find(c => norm(c.name).includes(norm(clusterNameRaw)) || norm(clusterNameRaw).includes(norm(c.name)));
                    if (!targetCluster) regionError = `Cluster "${clusterNameRaw}" not found in region "${targetRegion.name}"`;
                  } else {
                    targetCluster = rc[0];
                    if (!targetCluster) regionError = `No cluster found for region "${targetRegion.name}"`;
                  }
                }
              } else regionError = 'Missing region — add a region, region_id, or region_cluster column';
            }
          }

          if (!targetRegion || !targetCluster) {
            results.errors.push(`Row ${i + 1}: ${regionError}`);
            results.skipped++;
            continue;
          }
          ...
          try {
            await storage.createExamCenter({
              ...
              capacity: row['capacity'] ? parseInt(row['capacity']) : 500,
              isActive: true,
            } as any);
            results.created++;
          } catch (createErr: any) {
            if (createErr.message?.includes('unique') || createErr.message?.includes('duplicate')) {
              results.errors.push(`Row ${i + 1}: Code "${code}" already exists — skipped`);
            } else {
              results.errors.push(`Row ${i + 1}: ${createErr.message}`);
            }
            results.skipped++;
          }
        }
...
${JSON.stringify(schoolsForAI, null, 2)}`;
  ...
  app.get("/api/centers/:id/dashboard", isAuthenticated, async (req, res) => {
    try {
      ...
      // Build timetable with a 4-step fallback:
      // 1. Published exam_schedules for the active year
      // 2. Any exam_schedules for the active year (unpublished included)
      // 3. exam_timetable for the active year
      // 4. exam_timetable matching the year's grades (any year) — handles grade/year mismatch
      const allSubjects = await storage.getAllSubjects();
      let timetable: any[] = [];

      const scheduleToRow = (entry: any) => {
      ...
      const timetableToRow = (entry: any) => {
        const subject = allSubjects.find((s: any) => s.id === entry.subjectId);
      ...
      if (yearId) {
        // Step 1: published exam_schedules for active year
        const publishedSchedules = await storage.getExamSchedules({ examYearId: yearId, isPublished: true });
        if (publishedSchedules.length > 0) {
          timetable = publishedSchedules.map(scheduleToRow);
        } else {
          // Step 2: any exam_schedules for active year (unpublished included)
          const allSchedules = await storage.getExamSchedules({ examYearId: yearId });
          if (allSchedules.length > 0) {
            timetable = allSchedules.map(scheduleToRow);
          } else {
            // Step 3: exam_timetable for active year
            const ttByYear = await storage.getTimetableByExamYear(yearId);
            if (ttByYear.length > 0) {
              timetable = ttByYear.map(timetableToRow);
            } else {
              // Step 4: exam_timetable for matching grades, any year
              const yearGrades: number[] = (activeExamYear as any)?.grades ?? [];
              if (yearGrades.length > 0) {
                const ttByGrade = await storage.getTimetableByGradeAnyYear(yearGrades);
                timetable = ttByGrade.map(timetableToRow);
              }
            }
          }
        }
