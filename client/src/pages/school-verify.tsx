...
export default function SchoolVerify() {
  ...
  const [, params] = useRoute("/school-verify/:token");
  const token = params?.token || "";
  const [showPassword, setShowPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);
  const [setupComplete, setSetupComplete] = useState(false);

  const { data: schoolInfo, isLoading, error } = useQuery<SchoolInfo>({
    queryKey: ['/api/schools/verify-info', token],
    enabled: !!token,
  });
  ...
  const setupMutation = useMutation({
    mutationFn: async (data: SetupFormData) => {
      return apiRequest("POST", `/api/schools/verify/${token}/complete`, {
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Your school account has been verified and activated. You can now log in.",
      });
    },
    onError: (error: any) => {
      toast({
        ...
        <main className="container mx-auto px-4 py-12">
          <Card className="max-w-md mx-auto">
            <CardHeader className="text-center">
              ...
              <CardDescription>
                This verification link is invalid or has expired. Verification links are valid for 2 hours after being sent.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="bg-muted p-4 rounded-lg">
                ...
                <ul className="text-sm text-muted-foreground space-y-1">
                  <li>Contact your school administrator to request a new verification email</li>
                  <li>Make sure to complete verification within 2 hours of receiving the email</li>
                </ul>
              </div>
              <Link href="/">
  ...
  if (schoolInfo.isAlreadyVerified) {
    return (
      <div className="min-h-screen bg-background">
        <header className="border-b">
          <div className="container mx-auto px-4 py-4 flex items-center gap-3">
        ...
        <main className="container mx-auto px-4 py-12">
          <Card className="max-w-md mx-auto">
            <CardHeader className="text-center">
              ...
              <CardDescription>
                This school has already been verified and the account has been set up.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
        ...
        <main className="container mx-auto px-4 py-12">
          <Card className="max-w-md mx-auto">
            <CardHeader className="text-center">
              ...
              <CardDescription>
                Your school account has been verified and activated. You can now log in to access the examination portal.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="bg-muted p-4 rounded-lg">
      ...
      <main className="container mx-auto px-4 py-12">
        <Card className="max-w-md mx-auto">
          <CardHeader className="text-center">
            ...
            <CardTitle>Complete School Verification</CardTitle>
            <CardDescription>
              Set up your login credentials to activate your school account
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="bg-muted p-4 rounded-lg mb-6">
              ...
              <div className="space-y-1 text-sm">
                <p><span className="text-muted-foreground">School:</span> {schoolInfo.name}</p>
                <p><span className="text-muted-foreground">Registrar:</span> {schoolInfo.registrarName}</p>
                <p><span className="text-muted-foreground">Email:</span> {schoolInfo.email}</p>
              </div>
            </div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        This will be used to log in to your school dashboard
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
