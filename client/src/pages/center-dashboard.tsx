...
function TimetableTab({ timetable, printInfo, schoolGrades }: {
  ...
  const [activeGrade, setActiveGrade] = useState<string>(
    String(visibleGrades[0] ?? allGrades[0] ?? "")
  );

  // Keep activeGrade in sync when visible grades list changes (e.g. data loads)
  useEffect(() => {
    if (visibleGrades.length > 0 && !visibleGrades.map(String).includes(activeGrade)) {
      setActiveGrade(String(visibleGrades[0]));
    }
  }, [visibleGrades.join(","), activeGrade]);

  const handlePrint = (gradeFilter?: number) => {
    const filtered = gradeFilter != null ? timetable.filter(e => e.grade === gradeFilter) : timetable;
    ...
    const gradeTitle = gradeFilter != null ? ` — ${gradeLabel(gradeFilter)}` : "";
    const rows = sortedDates.map(date => {
      const label = new Date(date + "T00:00:00").toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
      ...
      return `<div class="day-block">
          ...
          <table><thead><tr><th>Time</th><th>Subject</th><th>Type</th><th>Duration</th></tr></thead>
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Timetable</title>
      ...
      ${printInfo ? buildPrintHeader(printInfo, `Examination Timetable${gradeTitle}`, logoSrc) : `<h2>Examination Timetable${gradeTitle}</h2>`}
      ${rows || "<p>No published schedule entries.</p>"}
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <h3 className="text-lg font-medium mb-2">No Published Schedule</h3>
          <p className="text-muted-foreground text-sm">
            No exam schedules have been published yet. Schedules are managed from the Exam Scheduling page.
          </p>
        </CardContent>
      </Card>
    );
  ...
  if (visibleGrades.length === 0) {
    return (
      <Card>
        <CardContent className="py-12 text-center">
          <Calendar className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
          <h3 className="text-lg font-medium mb-2">No Timetable for Your Classes</h3>
          <p className="text-muted-foreground text-sm">
            Your school does not have enrolled students in any of the examination classes with a published schedule.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className="space-y-4">
      ...
      {visibleGrades.map(g => {
        ...
        const byDate = gradeEntries.reduce((acc, e) => {
          if (!acc[e.examDate]) acc[e.examDate] = [];
          acc[e.examDate].push(e);
          return acc;
        }, {} as Record<string, typeof gradeEntries>);
          ...
          return (
            <Card key={g}>
              <CardContent className="py-10 text-center">
                <p className="text-muted-foreground text-sm">No published schedule entries for {gradeLabel(g)}.</p>
              </CardContent>
            </Card>
          );
        }
        ...
        return (
          <div key={g} className="space-y-3">
            ...
            <div className="flex items-center gap-2 px-1">
              <Badge variant="secondary" className="text-xs">{gradeEntries.length} session{gradeEntries.length !== 1 ? "s" : ""}</Badge>
              <Badge variant="outline" className="text-xs">{sortedDates.length} exam day{sortedDates.length !== 1 ? "s" : ""}</Badge>
            </div>
            ...
            {sortedDates.map((date, dateIndex) => {
              ...
              return (
                <Card key={date} className={`border ${colors.card} overflow-hidden`}>
                  <CardHeader className={`pb-2 pt-3 px-4 ${colors.header}`}>
                    <CardTitle className="text-sm font-semibold flex items-center gap-2">
                      <Calendar className="w-3.5 h-3.5" />
                      {new Date(date + "T00:00:00").toLocaleDateString(isRTL ? "ar-EG" : "en-US", {
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className={`${colors.tableHead} border-b`}>
                          <TableHead className="ps-4 text-xs">Time</TableHead>
                          <TableHead className="text-xs">Subject</TableHead>
                          <TableHead className="text-xs">Type</TableHead>
                          <TableHead className="pe-4 text-xs">Duration</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {dateEntries.map(entry => (
                          <TableRow key={entry.id}>
                            <TableCell className="font-medium ps-4 py-2.5 text-sm whitespace-nowrap">
                              ...
                              {entry.subjectName ? (
                                <div>
                                  ...
                                  {entry.subjectArabicName && entry.subjectArabicName !== entry.subjectName && (
                                    <span className="text-muted-foreground block text-xs" dir="rtl">
                            ...
                            <TableCell className="py-2.5">
                              {entry.subjectType ? (
                                <Badge variant="outline" className="text-xs capitalize">{entry.subjectType}</Badge>
                              ) : <span className="text-muted-foreground text-xs">—</span>}
                            </TableCell>
                            <TableCell className="py-2.5 pe-4 text-muted-foreground text-sm">
