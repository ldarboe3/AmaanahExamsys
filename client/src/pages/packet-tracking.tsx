...
function AutoGenerateDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const previewMutation = useMutation({
    mutationFn: () => apiRequest("POST", "/api/exam-packets/auto-generate", {
      examYearId, grade, bufferPercent, regionId: regionId || null, previewOnly: true,
    }).then(r => r.json()),
  ...
  const generateMutation = useMutation({
    mutationFn: () => {
      ...
      return apiRequest("POST", "/api/exam-packets/auto-generate", {
        examYearId, grade, bufferPercent, regionId: regionId || null, previewOnly: false,
      }).then(r => r.json());
    },
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        {step === "configure" && (
          <div className="space-y-4 py-1">
            ...
            {/* Auto-update notice */}
            <div className="flex items-start gap-3 py-2 px-3 rounded-md border border-teal-200 dark:border-teal-800 bg-teal-50/60 dark:bg-teal-950/20">
              <CheckCircle className="w-4 h-4 text-teal-600 dark:text-teal-400 shrink-0 mt-0.5" />
              <div>
                <p className="text-sm font-medium text-teal-800 dark:text-teal-200">Smart Sync Mode</p>
                <p className="text-xs text-teal-700 dark:text-teal-400">
                  Existing packets will have their paper count refreshed based on current enrolment. Missing packets will be created fresh.
                </p>
              </div>
            </div>
          ...
          <div className="space-y-4 py-1">
            ...
            <div className="rounded-md border bg-muted/30 p-3 space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                ...
                <div className="bg-blue-50 dark:bg-blue-950/30 rounded-sm p-2">
                  <p className="text-xl font-bold text-blue-700 dark:text-blue-300">{preview.wouldUpdate}</p>
                  <p className="text-[10px] text-muted-foreground leading-tight">To<br/>Update</p>
                </div>
              </div>
            ...
            <div className="flex justify-between gap-2 pt-2 border-t">
              ...
              <Button
                disabled={preview.wouldCreate === 0 && preview.wouldUpdate === 0}
              ...
              >
                ...
                {preview.wouldCreate === 0 && preview.wouldUpdate === 0
                  ? "Nothing to Sync"
                  : preview.wouldCreate > 0 && preview.wouldUpdate > 0
                    ? `Create ${preview.wouldCreate} + Update ${preview.wouldUpdate}`
                    : preview.wouldCreate > 0
                      ? `Create ${preview.wouldCreate} Packets`
                      : `Update ${preview.wouldUpdate} Packets`}
              </Button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4 py-1">
            ...
            <div className={`rounded-md border p-4 text-center space-y-2 ${
              ...
              <div className="flex justify-center gap-6">
                {result.created > 0 && (
                  <div>
                    <p className="text-3xl font-bold text-teal-700 dark:text-teal-300">{result.created}</p>
                    <p className="text-xs font-medium text-teal-600 dark:text-teal-400">created</p>
                  </div>
                )}
                {result.updated > 0 && (
                  <div>
                    <p className="text-3xl font-bold text-blue-700 dark:text-blue-300">{result.updated}</p>
                    <p className="text-xs font-medium text-blue-600 dark:text-blue-400">updated</p>
                  </div>
                )}
              ...
              <p className="text-sm font-medium text-emerald-600 dark:text-emerald-400">
                packets synced successfully
              </p>
              <p className="text-xs text-muted-foreground">
                {result.subjectCount} subjects · {result.centerCount} centers · Grade {result.grade}
              </p>
            </div>
