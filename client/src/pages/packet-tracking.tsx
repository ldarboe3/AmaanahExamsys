  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) handleClose(); }}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-1">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-md border bg-teal-50 dark:bg-teal-950/30 p-3">
                <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Examination Class</p>
              ...
              <div className="rounded-md border bg-muted/30 p-3">
                <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Subject Source</p>
                <p className="text-sm font-semibold">{preview.timetableBased ? "Published Timetable" : "All Grade Subjects"}</p>
                <p className="text-xs text-muted-foreground">{preview.timetableBased ? "Based on scheduled exams" : "No published timetable found"}</p>
              </div>
            </div>

            {/* Totals */}
            <div className="rounded-md border bg-muted/30 p-3 space-y-3">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Generation Preview</p>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="bg-background rounded-sm p-2.5">
                  <p className="text-2xl font-bold">{preview.totalSubjects}</p>
                  <p className="text-xs text-muted-foreground">Subjects</p>
                </div>
                <div className="bg-background rounded-sm p-2.5 flex items-center justify-center">
                  <p className="text-2xl font-bold text-muted-foreground">×</p>
                </div>
                <div className="bg-background rounded-sm p-2.5">
                  <p className="text-2xl font-bold">{preview.totalCenters}</p>
            ...
            {preview.subjects.length > 0 && (
              <div className="space-y-1.5">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-1.5">
                  ...
                  Grade {preview.grade} Subjects ({preview.totalSubjects})
                </p>
                <div className="rounded-md border bg-background divide-y max-h-32 overflow-y-auto text-sm">
                  {preview.subjects.map(s => (
                    <div key={s.id} className="px-3 py-1.5 flex justify-between items-center gap-2">
                      <span>{s.name}</span>
                      {s.arabicName && s.arabicName !== s.name && (
                        <span className="text-muted-foreground text-xs" dir="rtl">{s.arabicName}</span>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {/* Eligible Centers list */}
            {preview.eligibleCenters.length > 0 && (
              <div className="space-y-1.5">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-1.5">
                  <Building2 className="w-3.5 h-3.5 text-teal-600" />
                  Eligible Centers ({preview.totalCenters}) — have Grade {preview.grade} students
                </p>
                <div className="rounded-md border bg-background divide-y max-h-32 overflow-y-auto text-sm">
                  {preview.eligibleCenters.map(c => (
            ...
            {preview.centersWithNoStudents.length > 0 && (
              <div className="space-y-1.5">
                ...
                <div className="rounded-md border border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/20 divide-y max-h-24 overflow-y-auto text-sm">
            ...
            <div className="flex justify-between gap-2 pt-2 border-t">
              ...
              >
                ...
                Generate {preview.wouldCreate} Packets
              </Button>
            </div>
          </div>
        )}
        ...
        {step === "done" && result && (
          <div className="space-y-4 py-1">
            <div className="rounded-md bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 p-5 text-center space-y-2">
              ...
              {result.skipped > 0 && (
                <p className="text-xs text-muted-foreground">{result.skipped} duplicates were skipped</p>
              )}
            </div>

            <div className="rounded-md border bg-muted/30 p-3 space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Final Summary</p>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="bg-background rounded-sm p-2.5">
                  <p className="text-lg font-bold text-teal-700 dark:text-teal-300">Grade {result.grade}</p>
              ...
              {result.timetableBased && (
                <p className="text-xs text-center text-teal-600 dark:text-teal-400">Subjects sourced from published timetable</p>
              )}
            </div>
