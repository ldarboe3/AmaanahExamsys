...
function AutoGenerateDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const { data: timetable = [] } = useQuery<ExamTimetable[]>({
    queryKey: ["/api/timetable", examYearId],
    queryFn: () => fetch(`/api/timetable?examYearId=${examYearId}`).then(r => r.json()),
    enabled: !!examYearId,
  });

  // Timetable-based subject count for selected year (published entries only)
  const timetabledSubjectIds = examYearId
    ? [...new Set((timetable as any[]).filter((s: any) => s.isPublished).map((s: any) => s.subjectId))]
    : [];
        ...
        {result ? (
          <div className="space-y-4 py-2">
            <div className="rounded-md bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 p-4 text-center space-y-1">
              <p className="text-3xl font-bold text-emerald-700 dark:text-emerald-300">{result.created}</p>
              <p className="text-sm text-emerald-600 dark:text-emerald-400">packets created successfully</p>
              {result.skipped > 0 && (
                <p className="text-xs text-muted-foreground">{result.skipped} already existed and were skipped</p>
              )}
              <p className="text-xs text-muted-foreground pt-1">
                {result.subjectCount} subjects × {result.centerCount} centers
                {result.timetableBased && " — sourced from published timetable"}
              </p>
            </div>
            <p className="text-xs text-muted-foreground text-center">
              Each packet has a unique barcode and auto-generated security seal number. Print labels from the Packets table.
            </p>
          ...
          <div className="space-y-5 py-1">
            ...
            <div className="rounded-md border bg-muted/30 p-3 space-y-2 text-sm">
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                ...
                <div className="rounded-sm bg-background p-2">
                  <p className="text-lg font-bold">{scopedCenters.length}</p>
