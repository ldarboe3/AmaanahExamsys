...
function AutoGenerateDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () => apiRequest("POST", "/api/exam-packets/auto-generate", {
      examYearId, skipExisting, bufferPercent,
    }).then(r => r.json()),
    onSuccess: (data: any) => {
      setResult(data);
      queryClient.invalidateQueries({ queryKey: ["/api/exam-packets"] });
  ...
  const expectedPackets = subjects.length * centers.length;
    ...
    <Dialog open={open} onOpenChange={result ? () => { setResult(null); onClose(); } : onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Automatically creates one packet per subject per exam center, with paper counts calculated from enrolled student numbers.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-5 py-1">
            ...
            <div className="rounded-md border bg-muted/30 p-3 space-y-2 text-sm">
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                ...
                <div className="rounded-sm bg-teal-50 dark:bg-teal-950/40 p-2">
                  <p className="text-lg font-bold text-teal-700 dark:text-teal-300">{expectedPackets}</p>
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              >
                ...
                {mutation.isPending ? `Generating ${expectedPackets} packets…` : `Generate ${expectedPackets} Packets`}
              </Button>
            </div>
          </div>
        )}
